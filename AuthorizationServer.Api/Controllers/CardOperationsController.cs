    ...
    public class CardOperationsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> CreateJob([FromBody] JobModel entity) //Todo: Create the model for this entity
        {
            ...
            var newJob = new Job()
            {
                ...
                JobNameId = entity.JobNameId,
                SidSectorId = entity.SidSectorId,
            ...
            var lastCreatedJob = _repository.Jobs.Where(m => m.JobName == entity.JobName && m.JobNameId == entity.JobNameId).OrderByDescending(p => p.Id).ToList().FirstOrDefault();

...
namespace AuthorizationServer.Api.Controllers
{
    ...
    public class SidimClientsController : BaseApiController
    {
        ...
        public IQueryable<Job> PendingJobs()
        {
            ...
            return _repository.Jobs.Where(a => a.JobStatusId == jobStatus.Id);
        }
        ...
        {
            return _repository.JobTrackers.Where(a => a.IsCompleted == false && a.IsFlag == false);
        }
        ...
        {
            var jobTracker = _repository.JobTrackers.Where(a => a.Id == jobTrackerId);
            return _repository.Jobs.Where(a => jobTracker.Any(b => b.JobId == a.Id));
        }
        ...
        {
            //var jobBatchTracker = _repository.JobBatchTrackers.Include("JobTracker").Where(a => a.JobTracker.Id == jobTrackerId);
        ...
        public IQueryable<CardIssuanceLog> CardIssuanceLogByJobId(int jobId)
        {
            ...
            var jobTrackers = _repository.JobTrackers.Include("Job").Where(a => a.Job.Id == jobId);
            return _repository.CardIssuanceLogs.Where(a => jobTrackers.Any(b => b.Id == a.JobTrackerId));
        }

        [HttpGet]
        public IQueryable<CardIssuanceLog> CardIssuanceLogByTrackerId(int jobTrackerId)
        {
            return _repository.CardIssuanceLogs.Where(a => a.JobTrackerId == jobTrackerId);
        }
        ...
        public IQueryable<Job> IncompleteJobs()
        {
            ...
            return _repository.Jobs.Where(j => j.JobStatusId == jobStatusPending.Id || j.JobStatusId == jobStatusQueue.Id || j.JobStatusId == jobStatusWIP.Id || j.JobStatusId == jobStatusFlagged.Id);
        }

        [HttpGet]
        public IQueryable<JobTracker> JobTrackerByJobId(int jobId)
        {
            return _repository.JobTrackers.Where(a => a.JobId == jobId);
        }

        [HttpGet]
        public IQueryable<JobVariant> JobVariantByJobId(int jobId)
        {
            return _repository.JobVariants.Where(a => a.JobId == jobId);
        }
        ...
        {
            return _repository.CardIssuanceLogs;
        }
        ...
        public IQueryable<JobTracker> CardEngrIncomingPersos()
        {
            ...
            return _repository.JobTrackers.Where(j => j.FirstJobRunId == jobStatusQueue.Id && j.InventoryId == jobStatusCompleted.Id);
        }
        ...
        public IQueryable<JobTracker> CardEngrResumeNewPersos()
        {
            ...
            //var results = _repository.JobTrackers.Where(p => (partialCardIssuance.Any(p2 => p2.JobId == p.JobId && p.CardEngrResumeId == jobStatusQueue.Id && p.FirstJobRunId == jobStatusCompleted.Id && p.QAId == jobStatusCompleted.Id)));

            var jobHasWaste = _repository.JobSplitCEAnalysis.Where(j => j.QuantityHeld > 0 || j.QuantityBad > 0);
            var resumableJobs = _repository.JobTrackers.Where(j => j.CardEngrResumeId == jobStatusQueue.Id);

            var wasteResumables = _repository.JobTrackers.Where(j => j.CardEngrResumeId == jobStatusWIP.Id);

            var wasteToDisplay = jobHasWaste.Where(w => wasteResumables.Any(r => r.Id == w.JobTrackerId));

            var result = _repository.JobTrackers.Where(j => (wasteToDisplay.Any(w => w.JobTrackerId == j.Id)) || (resumableJobs.Any(r => r.Id == j.Id)));
            //var result = resumableJobs.Where(j => (jobHasWaste.Any(w => w.JobTrackerId == j.Id) && j.CardEngrResumeId == jobStatusWIP.Id));
        ...
        public IQueryable<JobTracker> CardEngrResumePendingPersos()
        {
            ...
            return _repository.JobTrackers.Where(j => j.FirstJobRunId == jobStatusCompleted.Id && j.QAId == jobStatusCompleted.Id && j.CardEngrResumeId == jobStatusWIP.Id);
        }
        ...
        public IQueryable<ClientVaultReport> ClientVaultReportByTrackerId(int trackerId)
        {
            ...
            if (tracker != null)
            {
                var jobVariant = _repo.FindJobVariantByJobId(tracker.JobId);
        ...
        {
            var jobStatusQueue = _repo.FindJobStatusByName("Queue");
            return _repository.JobTrackers.Where(j => j.PrintingId == jobStatusQueue.Id);
        }
        ...
        public IQueryable<JobTracker> PrintAnalysis()
        {
            var jobStatusWIP = _repo.FindJobStatusByName("WIP");
            var jobStatusCompleted = _repo.FindJobStatusByName("Completed");
            var jobTracker = _repository.JobTrackers.Where(j => j.PrintingId == jobStatusWIP.Id && j.PrintQAId == jobStatusCompleted.Id);
        ...
        public IQueryable<JobTracker> PrintDeliverables()
        {
            var jobStatusWIP = _repo.FindJobStatusByName("WIP");
            var jobStatusCompleted = _repo.FindJobStatusByName("Completed");
            var jobTracker = _repository.JobTrackers.Where(j => j.PrintingId == jobStatusWIP.Id && j.PrintQAId == jobStatusCompleted.Id && j.PrintQCId == jobStatusWIP.Id);
            var jobSplitAnalysis = _repository.JobSplitPrintCEAnalysis;
            return jobTracker.Where(a => jobSplitAnalysis.Any(b => b.JobSplit.JobTrackerId == a.Id));
        }
        ...
        public IQueryable<JobTracker> QAIncomingPersos()
        {
            ...
            return _repository.JobTrackers.Where(a => jobSplit.Any(b => b.JobTrackerId == a.Id && b.IsQACompleted == false));
        ...
        {
            var jobStatusQueue = _repo.FindJobStatusByName("Queue");
            return _repository.JobTrackers.Where(j => j.QCId == jobStatusQueue.Id);
        }
        ...
        {
            var jobStatusWip = _repo.FindJobStatusByName("WIP");
            return _repository.JobTrackers.Where(j => j.QCId == jobStatusWip.Id);
        }
        ...
        {
            var jobStatusQueue = _repo.FindJobStatusByName("Queue");
            return _repository.JobTrackers.Include("Job").Where(a => a.DispatchId == jobStatusQueue.Id);
        }
        ...
        {
            var jobStatusWIP = _repo.FindJobStatusByName("WIP");
            return _repository.JobTrackers.Where(a => a.PrintingId == jobStatusWIP.Id);
        }
        ...
        public IQueryable<JobTracker> MAIncomingJobs()
        {
            // Todo:: Previous Task must me completed
            var jobStatusQueue = _repo.FindJobStatusByName("Queue");
            return _repository.JobTrackers.Where(j => j.MailingId == jobStatusQueue.Id);
        }
        ...
        public IQueryable<JobTracker> MAPendingDeliverys()
        {
            // Todo:: Previous Task must me completed
            var jobStatusWIP = _repo.FindJobStatusByName("WIP");
            return _repository.JobTrackers.Where(j => j.MailingId == jobStatusWIP.Id);
        }
        ...
        {
            return _repository.SidProducts;
        }
        ...
        {
            return _repository.SidProducts.Where(a => a.SidClientId == clientId);
        }
        ...
        public IQueryable<Job> JobByCardIssuanceId(int stockReportId)
        {
            ...
            return _repository.Jobs.Where(b => cardIssuance.Any(c => c.JobId == b.Id));
        }
        ...
        {
            var jobStatus = _repo.FindJobStatusByName("Pending");
            return _repository.Jobs.Where(a => a.JobStatusId == jobStatus.Id && a.SidClientId == clientId);
        }
        ...
        public object Lookups()
        {
            ...
            var sidProducts = _repository.SidProducts;

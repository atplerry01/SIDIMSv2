(function () {
    ...
    function RepositoryResourceJob($routeParams, common, authService, model, AbstractRepository) {
        ...
        // ServerJobs
        function getServerJobQueues(forceRemote, page, size, nameFilter) {
            var self = this;
            var entity;
            var orderBy;
            var predicate = null;
            ...
            function querySucceeded(data) {
                entity = data.results;
                //self.log('Retrieved [ServerJobQueue Partials] from remote data source', entity.length, true);
                return getByPage();
            }
            ...
            function getByPage() {
                ...
                var newEntities = EntityQuery.from('ServerJobQueue')
                    .where(predicate)
                    .orderBy(orderBy)
                    .take(take).skip(skip)
                    .using(self.manager)
        ...
        function getServerJobFilteredCount(nameFilter) {
            ...
            var entities = EntityQuery.from(entityNames.serverJobQueue)
                    .where(predicate)
                    .using(this.manager)
                    .executeLocally();
        ...
        function getJobTrackers(forceRemote, page, size, nameFilter) {
            ...
            //if (self._areItemsLoaded() && !forceRemote) {
            //    //entity = self._getAllLocal(entityNames.jobTracker, orderBy);
            //    //return self.$q.when(entity);
            //    return self.$q.when(getByPage());
            //}

            return EntityQuery.from('JobTrackers')
                .select('id, jobId, cardOpsId, inventoryId, printingId, printQAId, printQCId, cardEngrId, qAId, firstJobRunId, cardEngrResumeId, qCId, mailingId, dispatchId, mAudId, customerServiceId, tAT, isFlag')
                .where(predicate)
                .orderBy(orderBy)
            ...
            function getByPage() {
                ...
                var newEntities = EntityQuery.from('JobTracker')
                    .where(predicate)
                    .orderBy(orderBy)
                    .take(take).skip(skip)
                    .using(self.manager)
        ...
        function getJobTrackerFilteredCount(nameFilter) {
            ...
            var entities = EntityQuery.from('JobTracker')
                    .where(predicate)
                    .using(this.manager)
                    .executeLocally();
        ...
        function getJobs(forceRemote, page, size, nameFilter) {
            ...
            return EntityQuery.from('Jobs')
                .select('id, jobName, sidSectorId, sidClientId, sidCardTypeId, remarkId, serviceTypeId, jobStatusId, quantity')
                .orderBy(orderBy)
        ...
        function getIncompleteJobs(forceRemote) {
            ...
            return EntityQuery.from('IncompleteJobs')
                .select('id, jobName, sidSectorId, sidClientId, remarkId, serviceTypeId, jobStatusId, quantity')
                .orderBy(orderBy)
                .toType(entityNames.job)
                .using(self.manager).execute()
                .then(querySucceeded, self._queryFailed);
